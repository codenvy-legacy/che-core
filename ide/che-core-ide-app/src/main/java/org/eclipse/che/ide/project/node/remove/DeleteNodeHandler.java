...
public class DeleteNodeHandler {
    ...
    @Inject
    public DeleteNodeHandler(RunnerServiceClient runnerServiceClient,
    ...
    public Promise<Void> deleteAll(@NotNull List<ResourceBasedNode<?>> nodes, boolean needConfirmation) {
        ...
        if (!needConfirmation) {
            ...
            return chainNodes(promise, filteredNodes.iterator(), true);
        }
        ...
        return createFromAsyncRequest(new RequestCall<Void>() {
            ...
            public void makeCall(AsyncCallback<Void> callback) {
                ...
                dialogFactory.createConfirmDialog(localization.deleteDialogTitle(),
                                                  warningMessage,
                                                  onConfirm(nodes, callback, false),
    ...
    private Promise<Void> chainNodes(@NotNull Promise<Void> promise,
                                     ...
                                     final boolean skipCheckingRunnerStatus) {
        ...
        final Promise<Void> derivedPromise = promise.thenPromise(new Function<Void, Promise<Void>>() {
            ...
            public Promise<Void> apply(Void empty) throws FunctionException {
                if (node instanceof ProjectDescriptorNode && !skipCheckingRunnerStatus) {
                    return getRunningProcess((ProjectDescriptorNode)node).thenPromise(hasRunningProcess())
                                                                         .thenPromise(removeNode(node));
                } else {
                    return node.delete();
                }
            }
        ...
        return chainNodes(derivedErrorSafePromise, projectNodes, skipCheckingRunnerStatus);
    }
    ...
    private ConfirmCallback onConfirm(@NotNull final List<ResourceBasedNode<?>> node,
        ...
        return new ConfirmCallback() {
            ...
            public void accepted() {
                ...
                chainNodes(promise, node.iterator(), skipCheckingRunnerStatus).then(new Operation<Void>() {
                    @Override
    ...
    @NotNull
    private Promise<List<ApplicationProcessDescriptor>> getRunningProcess(@NotNull ProjectDescriptorNode node) {
        return createFromAsyncRequest(getRunningProcessRC(node));
    }

    @NotNull
    private RequestCall<List<ApplicationProcessDescriptor>> getRunningProcessRC(@NotNull final ProjectDescriptorNode node) {
        return new RequestCall<List<ApplicationProcessDescriptor>>() {
            @Override
            public void makeCall(AsyncCallback<List<ApplicationProcessDescriptor>> callback) {
                final String projectPath = node.getStorablePath();
                runnerServiceClient.getRunningProcesses(projectPath,
                                                        _callback(callback,
                                                                  dtoUnmarshaller.newListUnmarshaller(ApplicationProcessDescriptor.class)));
            }
        };
    }

    @NotNull
    private Function<List<ApplicationProcessDescriptor>, Promise<Boolean>> hasRunningProcess() {
        return new Function<List<ApplicationProcessDescriptor>, Promise<Boolean>>() {
            @Override
            public Promise<Boolean> apply(List<ApplicationProcessDescriptor> process) throws FunctionException {
                return Promises.resolve(Iterables.any(process, isProcessNewOrRunning()));
            }
        };
    }

    @NotNull
    private Function<Boolean, Promise<Void>> removeNode(@NotNull final ResourceBasedNode<?> node) {
        return new Function<Boolean, Promise<Void>>() {
            @Override
            public Promise<Void> apply(Boolean hasRunningProcess) throws FunctionException {
                if (hasRunningProcess) {
                    return Promises.reject(JsPromiseError.create(localization.stopProcessesBeforeDeletingProject()));
                }
    ...
    @NotNull
    private Predicate<ApplicationProcessDescriptor> isProcessNewOrRunning() {
        return new Predicate<ApplicationProcessDescriptor>() {
            @Override
            public boolean apply(ApplicationProcessDescriptor process) {
                return process.getStatus() == NEW || process.getStatus() == RUNNING;
            }
        };
    }
