...
public class JsoIntegerMap<T> extends JavaScriptObject implements IntegerMap<T> {
     ...
     * @return an empty map
     */
    public static native <T> JsoIntegerMap<T> create() /*-{
        return {};
    }-*/;

    protected JsoIntegerMap() {
    }
    ...
    public final native void iterate(IntegerMap.IterationCallback<T> cb) /*-{
        for (var key in this) {
            if (this.hasOwnProperty(key)) {
                cb.
                    @org.eclipse.che.ide.collections.IntegerMap.IterationCallback::onIteration(ILjava/lang/Object;)
