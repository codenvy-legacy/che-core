...
public class StackServiceTest {
    ...
    public void setUp() throws IOException, ConflictException {
        ...
        userPermission.put(USER_ID, Arrays.asList("read", "write"));
        GroupDescriptor group = newDto(GroupDescriptor.class).withName("user").withAcl(Arrays.asList("read", "write", "search"));
    ...
    public void nonSystemUserShouldNotCreateStackWithPublicPermissions() {
        ...
        String expectedErrorMessage = "User '" + USER_ID + "' doesn't have access to use 'public: search' permission";
        assertEquals(unwrapDto(response, ServiceError.class).getMessage(), expectedErrorMessage);
    }
    ...
    public void shouldThrowForbiddenExceptionWhenWeTryDeleteAlienStack() throws NotFoundException, ServerException {
        ...
        verify(stackDao).getById(STACK_ID);
        assertEquals(response.getStatusCode(), 403);
        String expectedErrorMessage = format("User '%s' doesn't have access to stack with id '%s'", USER_ID, STACK_ID);
        assertEquals(unwrapDto(response, ServiceError.class).getMessage(), expectedErrorMessage);
    }
