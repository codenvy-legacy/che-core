...
public interface PermissionsChecker {
    ...
    /**
     * Checks that user with id {@code userId} has access to {@code target} with given {@code permission}
     *
     * @param target
     ...
     */
    boolean hasAccess(Permissible target, String userId, String permission) throws ServerException;

    /**
     * Checks that {@code permissions} contains "public: search" permission {@link Group}
     *
     * @param permissions
     *          permissions which allows access to content source {@see PermissionDescriptor}
     *
     * @return {@code true}
     *          when {@link Group} list of {@code permissions} contains "public: search" group, otherwise returns {@code false}
     */
